...
public class DiskLruImageCache implements ImageCache  {
    ...
    private static final Logger logger = Logger.getLogger(DiskLruImageCache.class.getSimpleName());
        ...
        try {
            ...
            if( writeBitmapToFile( data, editor ) ) {               
                ...
                if ( BuildConfig.DEBUG ) {
                   logger.severe( "cache_test_DISK : image put on disk cache " + key );
                }
            } else {
                editor.abort();
                if ( BuildConfig.DEBUG ) {
                    logger.severe( "cache_test_DISK : ERROR on image put on disk cache " + key );
                }
            }   
        } catch (IOException e) {
            if ( BuildConfig.DEBUG ) {
    ...
    public Bitmap getBitmap( String key ) {
        ...
        if ( BuildConfig.DEBUG ) {
            ...
            logger.severe( "cache_test_DISK_ : " + (bitmap == null ? "will download image" + key : "image read from disk " + key));
        }
